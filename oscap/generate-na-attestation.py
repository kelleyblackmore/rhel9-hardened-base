...
CATEGORIES = [
    (r"^(grub2_|file_(owner|groupowner|permissions)_grub2|zipl_|uefi_)",
     "Boot loader",
    ...
    (r"^(partition_for_|encrypt_partitions|mount_option_)",
    ...
    (r"^(dconf_|gnome|gdm|package_(gdm|gnome))",
    ...
    (r"^(sysctl_|kernel_module_|grub2_.*_argument|coredump_|disable_users_coredumps)",
    ...
    (r"^(service_|timer_|socket_|systemd_)",
    ...
    (r"(firewalld|firewall|libreswan|ipsec|wireless|networkmanager|network_sniffer)",
    ...
    (r"(smartcard|opensc|pcsc|pam_pkcs11|certmap)",
     "Smart card / CAC",
    ...
    (r"(fips|crypto_fips|aide|rpm_verify|integrity)",
     "FIPS / host integrity",
    ...
    (r"(pam_faillock|pam_pwquality|password_pam|accounts_password|passwords_pam|account_pam|pam_wheel|pam_)",
     "PAM / password policy",
     "No interactive local logins or PAM authentication stack is used by a container base; identity and authentication are handled by the platform/IdP. Not applicable to a container image."),
    (r"^(accounts_|account_|no_empty_passwords|gid_|group_unique|use_pam_wheel)",
     "Local accounts",
    ...
    (r"(banner|motd|login_banner|dconf_.*banner)",
     "Login banners",
    ...
    (r"(subscription-manager|rng-tools|tuned|usbguard|chrony|ntp|time_)",
